...
const JudgeCard: React.FC<{ judge: Judge }> = ({ judge }) => (
  <div className="text-center p-6 bg-slate-900 rounded-2xl shadow-lg hover:shadow-2xl hover:shadow-violet-500/20 transition-all duration-300 ease-in-out transform hover:-translate-y-2 hover:scale-105 border border-slate-800">
    <img src={judge.imageUrl} alt={judge.name} className="w-32 h-32 rounded-full mx-auto mb-4 border-4 border-violet-700" />
    <h3 className="text-2xl font-bold text-slate-100">{judge.name}</h3>
